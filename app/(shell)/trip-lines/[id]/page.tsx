...
type EditableStop = { stop: Stop; stopType: StopUse };
...
export default function TripLineDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const remaining = useMemo(() => available.filter((stop) => !editStops.some((item) => item.stop.id === stop.id)), [available, editStops]);
  ...
  function openDirectionEditor(directionId: string, stations: TripLineStop[]) {
    ...
    setEditStops(stations.map((item) => ({ stop: item.station, stopType: item.stopType })));
    setPick("");
    setError(null);
  }
  ...
  function addStop() {
    ...
    if (!stop) return;
    setEditStops((items) => [...items, { stop, stopType: "BOTH" }]);
    setPick("");
  }
  ...
  async function saveDirection() {
    ...
    setSaving(true);
    const result = await updateTripLineDirectionStops(id, editingDirectionId, editStops.map((item) => ({ stopId: item.stop.id, stopType: item.stopType })));
    setSaving(false);
    if (!result.ok) return setError(result.message);
    setLine(result.data);
