...
export default function NewTripLinePage() {
  ...
  const [stopTypes, setStopTypes] = useState<Record<string, "BOARDING" | "LANDING">>({});
  ...
  function addStop() {
    ...
    if (!stop) return;
    setSelected((items) => [...items, stop]); setStopTypes((types) => ({ ...types, [stopTypeKey(direction, stop.id)]: types[stopTypeKey(direction, stop.id)] ?? "BOARDING" }));
    setPick("");
    setError(null);
  }
  ...
  function copyAndReverseOutbound() {
    ...
    setReturnStops([...outbound].reverse()); setStopTypes((types) => ({ ...types, ...Object.fromEntries(outbound.map((stop) => [stopTypeKey("return", stop.id), types[stopTypeKey("outbound", stop.id)] ?? "BOARDING"])) }));
    setDirection("return");
    setPick("");
    setError(null);
  }
  ...
  async function submit(event: React.FormEvent) {
    ...
    const result = await createTripLine({
      ...
      outboundStops: outbound.map((stop) => ({ stopId: stop.id, stopType: stopTypes[stopTypeKey("outbound", stop.id)] ?? "BOARDING" })),
      returnStops: returnStops.map((stop) => ({ stopId: stop.id, stopType: stopTypes[stopTypeKey("return", stop.id)] ?? "BOARDING" })),
    });
    setSaving(false);
