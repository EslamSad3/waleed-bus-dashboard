...
export default function NewTripLinePage() {
  ...
  const [stopTypes, setStopTypes] = useState<Record<string, "BOARDING" | "LANDING" | "BOTH">>({});
  ...
  const stopTypeKey = (routeDirection: Direction, stopId: string) => `${routeDirection}:${stopId}`;
  ...
  function addStop() {
    ...
    if (!stop) return;
    setSelected((items) => [...items, stop]); setStopTypes((types) => ({ ...types, [stopTypeKey(direction, stop.id)]: types[stopTypeKey(direction, stop.id)] ?? "BOTH" }));
    setPick("");
    setError(null);
  }
  ...
  function copyAndReverseOutbound() {
    ...
    setReturnStops([...outbound].reverse()); setStopTypes((types) => ({ ...types, ...Object.fromEntries(outbound.map((stop) => [stopTypeKey("return", stop.id), types[stopTypeKey("outbound", stop.id)] ?? "BOTH"])) }));
    setDirection("return");
    setPick("");
    setError(null);
  }
  ...
  async function submit(event: React.FormEvent) {
    ...
    const result = await createTripLine({
      ...
      outboundStops: outbound.map((stop) => ({ stopId: stop.id, stopType: stopTypes[stopTypeKey("outbound", stop.id)] ?? "BOTH" })),
      returnStops: returnStops.map((stop) => ({ stopId: stop.id, stopType: stopTypes[stopTypeKey("return", stop.id)] ?? "BOTH" })),
    });
    setSaving(false);
