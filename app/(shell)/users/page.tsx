...
import { createAdminUser, deleteAdminUser, fetchAdminUser, fetchAdminUsers, setAdminUserRoles, updateAdminUser, type AdminUser } from "@/lib/actions/users";
...
export default function UsersPage() {
  ...
  function resetForm() {
    setSelected(null); setName(""); setEmail(""); setPassword(""); setIsActive(true); setRoleSlugs([]); setError(null);
  }
  ...
  async function openEdit(user: AdminUser) {
    ...
    setSelected(current); setName(current.name ?? ""); setEmail(current.email ?? ""); setPassword(""); setIsActive(current.isActive);
  ...
  async function save() {
    ...
    if (!selected) {
      ...
      const profile = await updateAdminUser(selected.id, { name: name.trim() || undefined, isActive, ...(password ? { password } : {}) });
